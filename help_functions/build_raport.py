...
def build_raport(filename, month, year, username, id_user):
        ...
        report.add_paragraph(f"User: {username}")
        report.add_title("Data visualisation")
        report.add_paragraph(" Charts below shows the most important personal budget indicators.")
        piechart_category = plot_transactions_by_category(get_transactions_by_category(month,year, id_user))
        ...
        balance_chart =balance_over_time(get_balance_over_time(period = 'daily', user_id = id_user), period = 'daily')
        balance_chart_2 = balance_over_time(get_balance_over_time(period = 'monthly', user_id= id_user), period = 'monthly')
        ...
        income_vs_expense_chart = plot_incomes_vs_expenses(get_income_vs_expense(limit=5, user_id=id_user))
        ...
        top_expenses_chart = plot_top_n_expenses(get_top_expenses(limit=5, year=year, month=month, user_id=id_user))
        ...
        chart_savings_progress = plot_savings_progress(get_savings_progress(id_user))
        ...
        summary = get_monthly_summary(year, month, id_user)
        summary_table = pd.DataFrame([summary])  
        summary2 = get_monthly_summary(year, month-1 if month > 1 else 12, id_user)
